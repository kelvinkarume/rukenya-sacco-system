...
<div class="space-y-6">
    ...
    <div>
        ...
        <p class="text-gray-500 text-sm">
            Detailed report of all loans issued.
        </p>
    </div>
    ...
    <div class="bg-white p-4 rounded shadow overflow-x-auto">
        <table class="w-full table-fixed">
            ...
            <tbody>
                ...
                <tr>
                    <td class="px-4 py-2 text-left">{{ $loan->id }}</td>
                    <td class="px-4 py-2 text-left">{{ $loan->user->name ?? 'N/A' }}</td>
                    <td class="px-4 py-2 text-left">KES {{ number_format($loan->amount) }}</td>
                    <td class="px-4 py-2 text-left">{{ $loan->status }}</td>
                    <td class="px-4 py-2 text-left">{{ $loan->created_at->format('Y-m-d') }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
